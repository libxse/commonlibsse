...
namespace REX::INI
{
	namespace detail
	{
		...
		template <class T>
		void SettingSaveImpl(void* a_file, T& a_value, T& a_valueDefault, std::string_view& a_section, std::string_view& a_key);
	}
	...
	public:
		...
		{
			detail::SettingSaveImpl(a_file, this->m_value, this->m_valueDefault, m_section, m_key);
		}
